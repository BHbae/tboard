...
public class UserController extends HttpServlet {
	 ...
	 * @param response
	 * @throws IOException 
	 * http://localhost:8080/t_board/user/logout
	 */
	...
	private void handleSignin(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		User principal = userRepository.getUserByusernameAndPssword(username, password);
		
		if(principal != null && principal.getPassword().equals(password) ) {
			HttpSession session = request.getSession();
			session.setAttribute("principal", principal);
			
			// 302(브라우저) --> 사볼 서블릿 클래스(BoardController) -- (JSP 내부이동)
